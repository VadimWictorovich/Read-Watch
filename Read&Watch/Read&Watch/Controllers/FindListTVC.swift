...
class FindListTVC: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.textLabel?.text = (item.name ?? "Нет данных") + " " + "(\(item.year?.description ?? "Нет данных"))"

        //cell.textLabel?.text = "\(item.name ?? item.enName ?? item.alternativeName ?? item.names?.first ?? "") (\(yearString))"
        return cell
    }
